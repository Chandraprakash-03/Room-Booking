...
app.get('/api/get-patient-data', (req, res) => {
  ...
  app.post('/api/cancel-room', (req, res) => {
    const { roomNumber, cancellationReason } = req.body;
  
    const index = bookedRooms.findIndex(room => room.roomNumber === roomNumber);
  
    if (index !== -1) {
      const cancelledRoom = bookedRooms.splice(index, 1)[0];
      
      // Trigger message to patient (in a real system, this would involve an external service)
      console.log(`Message sent to ${cancelledRoom.phone}: Room booking cancelled. Reason: ${cancellationReason}`);
  
      res.json({ message: 'Room booking cancelled successfully', cancelledRoom });
    } else {
